def relationship_status(from_member, to_member, social_graph):
    ...
    print(from_member_following, to_member_following)
  ...
  def tic_tac_toe(board):
    ...
    while column_number < len(board[0]):
      win = True
      first_symbol = board[0][column_number]
      row_number = 0
      while row_number < len(board):
        if board[row_number][column_number] != first_symbol:
          win = False
    ...
    row_number = 0
    column_number = 0
    first_symbol = board[row_number][column_number]
    win = True
    while row_number < len(board):
      if board[row_number][column_number] != first_symbol:
          win = False
  ...
  def eta(first_stop, second_stop, route_map):
    ...
    travel_time = 0
    while True:
      if location == second_stop:
        break
      for route, minutes in route_map.items():
        if route[0] == location:
          location = route[1]
          travel_time += minutes['travel_time_mins']
          break
    return travel_time
